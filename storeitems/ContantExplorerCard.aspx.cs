        ...
        protected void LOADMORECHAPTERS(object sender, EventArgs e)
        {
            ...
            if (GetUserInfoCookie != null)
            {
                ...
                for (int c = (CSN + 1); c < (CEN + 1); c++)
                {
                    ...
                    if (c < CEN) { TheMangaPhotosF.InnerHtml += "<hr style=" + sc.ToString() + "margin:0 auto !important;height:4px;border-radius:1px;border-width:0;color:#ffffff;background-color:#FFFFFF;width:80%;opacity:0.0;margin:0px;margin-block:0px;" + sc.ToString() + ">"; }
                }
            }
            ThreIsMoreACard.Attributes["style"] = "display:none;";
            LoadChapters.Update();
